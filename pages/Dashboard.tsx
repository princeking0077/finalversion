...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    setResults(db.getUserResults(currentUser.id));
                    ...
                    {(['all', 'pending', 'completed'] as const).map(f => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all whitespace-nowrap ${
                                filter === f ? 'bg-teal-500 text-white shadow-lg' : 'text-slate-400 hover:text-white'
                            }`}
                        >
                            {f}
                        </button>
            ...
            {filteredTests.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredTests.map(test => {
                        ...
                        return (
                            <div key={test.id} className={`glass-card rounded-2xl p-6 border transition-all duration-300 flex flex-col h-full group ${
                                isCompleted 
