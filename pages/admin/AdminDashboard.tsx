...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    const userList = db.getUsers().filter(user => user.role === 'student');
    setUsers(userList);
    setAllTests(db.getTests());
  }, [navigate]);

  const refreshData = () => {
    setUsers(db.getUsers().filter(user => user.role === 'student'));
    setAllTests(db.getTests());
  };
    ...
    approvedStudents.forEach(u => {
      if (selectedStudents.includes(u.id)) {
        if (!u.enrolledCourses.includes(selectedCourse)) {
          u.enrolledCourses.push(selectedCourse);
          db.updateUser(u);
          count++;
        }
      }
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                                        isSelected || isEnrolled ? 'bg-indigo-500 border-indigo-500' : 'border-slate-600 bg-slate-800'
                                    }`}>
